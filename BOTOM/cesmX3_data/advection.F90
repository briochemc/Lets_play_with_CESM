...
!  SVN:$Id: advection.F90 80582 2016-09-18 13:27:40Z klindsay $
! !USES:
   use POP_KindsMod
   use POP_ErrorMod
   use POP_CommMod
...
!
!-----------------------------------------------------------------------
! sizing parameters for IRF additions  AMB
   integer (int_kind) :: IRF_NT
   character(len=10)  :: env_string 
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_ADV_TRACER(2),'ADVS',2,                        &
                    long_name='Vertically-Integrated S Advection Tendency',   &
                          ...
                          coordinates='TLONG TLAT time')
! AMB this should be ECOSYS_BEG,IAGE_END
   do n=3,nt
!    call get_environment_variable("IRF_NT", env_string)
!    read(env_string, '(I10)') IRF_NT
...
! AMB should be 1, IRF_NT instead of 1,nt
!   call get_environment_variable("IRF_NT", env_string)
!   read(env_string, '(I10)') IRF_NT
